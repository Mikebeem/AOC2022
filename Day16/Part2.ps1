...
$pathsCalculated.GetEnumerator() | Sort Value -Descending | Select-Object -First 8 | ForEach-Object {
...
while ($q.Count -gt 0) {
    ...
    if($minutesLeft -eq 0){
        $higestFound += @{$openValve = $pressure}
    }
    elseif($minutesLeft -gt 0){
        ...
        $pathsCalculated.GetEnumerator() | Sort Value -Descending | Where-Object { $openValve -notlike "*$($_.Key)*" } | Select-Object -First 4 | ForEach-Object {
            ...
            if(($minutesLeft-$count) -gt 0){
                $toQ = ("{0},{1},{2},{3}" -f $_.Key,($openValve + ";" + $_.Key), $pressureToQ,($minutesLeft-$count))
                $q.Enqueue($toQ)
            }
        }
    }
    # if($q.count % 1000 -eq 0){
    #     "Queueu is: {0}" -f $q.count
    #     "{0},{1},{2},{3}" -f $current, $openValve,$pressure,$minutesLeft
    # }
    if($valvesToOpenQ.Count -eq 0 -and $minutesLeft -ge 0){
        $minutesLeft = $clock
        $higestFound += @{$openValve = $pressure}
    }
}
$higestFound.Count
$topValue = 0
$higestFound.GetEnumerator() | Sort Value -Descending | Select-Object -First ($higestFound.Count / 10) | ForEach-Object {
    $left = $_

    $higestFound.GetEnumerator() | Sort Value -Descending | Select-Object -First ($higestFound.Count / 10) | ForEach-Object {
        $right = $_
        if((Compare-Object ($left.Key).Split(";") ($right.Key).Split(";") -IncludeEqual -ExcludeDifferent).Count -eq 0){
            if($topValue -lt ($left.Value + $right.Value)){
                $topValue = $left.Value + $right.Value
                $topValue
                $left.Key
                $right.Key
            }
