...
foreach($blueprint in $puzzleInput[0..2]){
    ...
    #0=oreBots, 1=ClayBots, 2=ObsBots, 3=Ore, 4=Clay, 5=Obs, 6=Geo, 7=Minutes
    ...
    while ($q.Count -gt 0) {
        ...
        elseif($oreRobots -lt $maxOreBots -and ($minutes+$oreTime) -le 16){
        ...
        if($clayRobots -lt $maxClayBots -and ($minutes+$clayTime) -le 28){
            #When clayrobots > 0, choose to build more
        ...
        if($clayRobots -ge 1 -and $obsidianRobots -eq 0 -and ($minutes+$obsidianCosts) -le 33){
        ...
        if($obsidianRobots -ge 1 -and $obsidianRobots -lt $maxObsidianBots){
            #When clayrobots, build obs robot
            if(($minutes+$obsidianCosts) -le 31){
                $q.Enqueue(@($oreRobots,$clayRobots,($obsidianRobots+1),($ore+($oreRobots*$obsidianCosts)-$obsidianOre),($clay+($clayRobots*$obsidianCosts)-$obsidianClay),($obs+($obsidianRobots*$obsidianCosts)),$geo,($minutes+$obsidianCosts)))
            }
        }
